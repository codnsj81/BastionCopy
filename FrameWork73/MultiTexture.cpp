...
int CMultiTexture::GetTexInfoCount(const TCHAR* pKey)
{
										  ...
										  const int& iCount /*= 0*/)
{
	...
	map<const TCHAR*, vector<TEX_INFO*>>::iterator Finditer = m_mapMultiTexture.begin();

	Finditer = find_if( m_mapMultiTexture.begin(), m_mapMultiTexture.end(), STRING_COMPARE(pKey) );

	if(Finditer == m_mapMultiTexture.end())
	{
		Finditer = m_mapMultiTexture.begin();
		return NULL;
	}
